    ...
    public class TipoArticulo: ITipoArticulo
    {
        ...
        public List<MdlTipoArticulo> GetList(int idTpArt, string Nombre)
        {
            ...
            using (var con = new OracleConnection(_config.CadenaConexion))
            {
                ...
                string cadena = "Select Id_Tipo_Articulo, Nombre, Descripcion, Fecha_Creacion, Usuario_Creacion from Tipo_Articulo Where 1 = 1";

                if (idTpArt != 0)
                {
                    cadena += $" and  id_tipo_articulo =  {idTpArt}";
                }
